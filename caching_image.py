...
class caching_image:
    ...
    RETURN_NAMES = ("original_image", "cached_image",)
    FUNCTION     = "caching_image"
    CATEGORY     = "caching_to_not_wast"



    def caching_image(self, *args, **kwargs):
        os.makedirs(CACHE_DIR, exist_ok=True)

        image_path = caching_image.reference_image_cache_hash(kwargs['original_image'], kwargs['identification'] + '_img_')

        if os.path.exists(image_path) and not kwargs['force_recreate']:
            return caching_image.return_cached_image(kwargs['original_image'], image_path)

        return caching_image.execute(self, args, image_path, kwargs)
    ...
    def reference_image_cache_hash(original_image, identification):
        ...
        return caching_image._resolve_path(image)
    ...
    def IS_CHANGED(self, original_image, executor, force_recreate, identification = None):
        image_path = caching_image._resolve_path(original_image)
        m = hashlib.sha256()
    ...
    def check_lazy_status(self, identification, original_image, force_recreate, executor=None):
        image_path = caching_image.reference_image_cache_hash(original_image, identification  + '_img_')

        if os.path.exists(image_path) and not force_recreate:
            return caching_image.return_cached_image(original_image, image_path)
